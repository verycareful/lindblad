...
Returns the build version string derived from the CMake `LINDBLAD_VERSION_LABEL` compile definition (e.g. `"R.1.11.2"`). The value tracks `LINDBLAD_VERSION_LABEL` in `CMakeLists.txt` and cannot drift from the project version.
