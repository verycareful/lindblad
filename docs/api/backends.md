...
Returns the build version string derived from the CMake `LINDBLAD_VERSION_LABEL` compile definition (e.g. `"1.1.28.1"`). The value tracks `LINDBLAD_VERSION_LABEL` in `CMakeLists.txt` and cannot drift from the project version.
