...
[![Version](https://img.shields.io/badge/version-R.1.10.3-blue?style=flat-square)](CHANGELOG.md)
...
| Version | Description |
|---|---|
| `R.1.10.3` | Visualiser ergonomics. New `QuantumCircuit::draw_to_file(path, mode, opts)` writes the rendered output directly to a file path, raising `std::runtime_error` on a failed open rather than silently truncating; saves the `#include <fstream>` + `std::ofstream` dance at every call site. New `lindblad_draw` CLI executable (built when `LINDBLAD_BUILD_APPS=ON`, default for top-level builds) renders QASM files (`--qasm3` to opt into QASM 3) or built-in demos (`--demo bell|ghz|parametric|tallbox|qft`) to stdout or `--output <path>`; respects `--mode`, `--ascii-safe`, `--show-clbits`, `--no-show-params`, `--param-format`, `--fold`, `--cell-px`, `--legend`. The Python bindings expose `qc.draw_to_file()` for consistency, though direct C++ remains the recommended path for any performance-sensitive workflow (the Python wrapper crosses the binding boundary and serialises through the GIL). 13 new tests in `DrawToFileTest` cover the round-trip against every backend, options propagation, default-mode behaviour, file content prologues, error path on unwritable destinations, and overwrite semantics. 1042 of 1042 tests passing |
| `R.1.10.2` | First patch for the visualiser: closes the five R.1.10.1 known failures and the two cosmetic gaps noted alongside them. `src/visualisation/render_latex.cpp` now consults `GateSymbol::latex_macro` from the Tier 1 catalogue, so daggered gates emit `\gate{S^{\dagger}}` and rotation gates emit `\gate{R_X\text{(π/2)}}` instead of `\gate{\text{S†}}` / `\gate{\text{RX(π/2)}}`. The override stores only the math-mode symbol; the renderer splits each `BoxPart` label at the first `(` so the parameter suffix continues to round-trip in `\text{...}`. `src/visualisation/render_ascii.cpp` now annotates conditional-gate c-wire crossings with the match value (`═╪=1═`), and skips the odd-parity column-width bump when the layer contains no centred glyph (control bullet, XOR target, SWAP arm, barrier) so even-length labels like `RXX(π/4)` fit without trailing padding. Three test-side issues from R.1.10.1 are also resolved: the non-contiguous-UNITARY layout test now matches against the actual `data_gate` value (`"U"` from the fixture label); the arbitrary-decimal format test uses an exactly-representable value (0.5) instead of `0.31415`; the HTML legend absence test looks for the `<div class="lb-legend">` element instead of the always-declared CSS class. 1029 of 1029 tests passing |
| `R.1.10.1` | Test suite for the circuit visualiser introduced in R.1.10.0. 194 new tests across 9 new suites (`DocumentLayoutTest`, `GateCatalogueTest`, `FormatParamsTest`, `PaletteTest`, `FoldingTest`, plus per-backend `AsciiFixtureTest`, `SvgFixtureTest`, `LatexFixtureTest`, `HtmlFixtureTest`). Coverage breadth: ASAP packing edge cases (BARRIER full-width breaks, non-contiguous UNITARY row reservation, conditional gate c-wire serialisation, MEASURE strut placement), Tier 1 / Tier 2 / Tier 3 catalogue completeness, CZ / CCZ all-CtrlBullet structure, TallBox handling for RXX / RYY / RZZ / RZX / ECR, every pi-snap table entry (signed pairs), ParamExpr precedence-aware paren wrapping, palette correctness across all four backends, ascii_safe portable fallback, and golden-file byte-exact comparison for 29 fixture-x-backend snapshots stored under `tests/golden/visualisation/`. A standalone `lindblad_visualiser_regen` binary regenerates the golden tree from the current implementation. Five tests are marked as known failures (3 test-side issues, 2 real implementation gaps in `render_latex.cpp` around `GateSymbol::latex_macro` lookup); all five are tracked for resolution in R.1.10.2. 1024 of 1029 tests passing |
