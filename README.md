...
[![Version](https://img.shields.io/badge/version-v1.9.2--alpha-orange?style=flat-square)](CHANGELOG.md)
...
- Current release: `v1.9.2-alpha` (build: wire `ising.cpp` and `dispatch.cpp` into `qpp_core` — `IsingHamiltonian` and `SoftDispatchResult` were compiled into no translation unit)
- Previous release: `v1.9.1-alpha` (fix: MSVC portability — delete non-copyable constructors on `VQE`/`QAOA`/`MAQAOA`, `M_PI` guards, `qpp::transpile()` replaces broken `preset_pass_manager` call, copy-assign `Estimator` removed from tests)
