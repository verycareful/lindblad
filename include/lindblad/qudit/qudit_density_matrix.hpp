...
public:
    ...
    // Judge this matrix's normalization under a validation policy.
    // Repair::Attempt renormalizes in place; without it Warn reports and
    // leaves the matrix as it is, Throw raises, and Ignore measures nothing,
    // so opting out costs one branch rather than a full pass. A matrix with
